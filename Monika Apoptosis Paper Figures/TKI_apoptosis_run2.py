...
y_steady = np.zeros([len(ymax), len(ymax)])
sens_kd = np.zeros([len(ymax), len(ymax)]) 

# simulating knockdowns one by one
for i in range(0, len(ymax)):
	print('Param #', str(i+1), 'of', str(len(ymax)))
	ymax_new = ymax.copy()
	# ymax_new = new maximum values of each species AFTER KD (0)
	ymax_new[i] = 0

	# knockdown under sorafetinib activation 
	# remember to use past baseline simulation's steady state values as this simulation's y0 values
	w[4] = 1
	treatment_kd = solve_ivp(TKI_apoptosis.ODEfunc, tspan, yend_treatment, args=(tau, ymax_new, w, n, EC50))
	# take SS values of sorafetinib activation
	yend_kd2 = treatment_kd.y.T[-1,:]

	# sensitivity output (KD with sor - sor at steady state)
	#sens_kd[:,i] = -(yend_kd2-sens_treat_kd)/(ymax_new[i]-ymax[i])*ymax[i]/(sens_treat_kd)
	sens_kd[:,i] = yend_kd2-sens_treat_kd

# if there are any non-real numbers, change them to 0 
for i in range(0,len(ymax)):
    for j in range(0, len(ymax)):
        if np.isnan(np.real(sens_kd[i,j]==1)):
            sens_kd[i,j] == 0
...
sns.heatmap(sens_kd, annot = False, cmap = 'seismic', vmin = vmin, vmax = vmax, xticklabels=speciesNames, yticklabels=speciesNames)
